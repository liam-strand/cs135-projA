...
def main():
    ...
    pipeline = sklearn.pipeline.Pipeline([
        ("bow_feature_extractor", CountVectorizer(min_df=10, max_df=0.97,
         ngram_range=(1, 1), stop_words="english", strip_accents="ascii")),
        ("classifier", sklearn.linear_model.LogisticRegression(
            C=1.0, max_iter=2000, random_state=101)),
    ])
    ...
    clf = sklearn.model_selection.RandomizedSearchCV(
        pipeline, distributions, n_iter=100, verbose=2)
...
def get_all_data_from_dir(dirname: str) -> tuple[list[str], list[str], list[str]]:
    ...
    y_train = load_data_from_file(os.path.join(
        dirname, "y_train.csv"), "is_positive_sentiment")
